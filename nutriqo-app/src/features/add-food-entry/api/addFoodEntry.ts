...
export async function addFoodEntry(
  ...
  try {
    ...
    const normalizedInput: AddFoodEntryInput = {
      ...
      protein: input.protein && input.protein > 0 ? Number(input.protein) : undefined,
      fats: input.fats && input.fats > 0 ? Number(input.fats) : undefined,
      carbs: input.carbs && input.carbs > 0 ? Number(input.carbs) : undefined,
    };
    ...
    // Отправляем только заполненные поля БЖУ
    const foodEntryData: Omit<EatenFoodType, 'id' | 'created_at' | 'updated_at'> = {
      name: normalizedInput.name,
      calories: normalizedInput.calories,
      protein: normalizedInput.protein ?? 0,
      fats: normalizedInput.fats ?? 0,
      carbs: normalizedInput.carbs ?? 0,
    ...
    logger.info(`Food entry created: ${savedEntry.id} for goal ${normalizedInput.goal_id}`);
    return savedEntry;
  } catch (error) {
    // Логирование и пробрасывание ошибки с деталями
